...
db %>% 
  ...
  ggplot(aes(y = fct_reorder(type_of_measure, desc(count)),
...
db %>% 
  ...
  geom_hline(yintercept = 0.376, color = "grey40", size = .5,
             linetype = "dashed") +
  coord_flip() + 
...
db %>% 
  ...
  ggplot(aes(x = fct_reorder(country, monto),
             y = monto)) +
  geom_col(color = "grey0") +
  geom_text(aes(x = fct_reorder(country, monto),
                y = monto,
                label = comma(monto, accuracy = 1)), hjust = -.2, size = 2.8) + 
  geom_hline(yintercept = 485, color = "grey40", size = .5,
             linetype = "dashed") +
  coord_flip() +
  scale_y_continuous(label = comma, limits = c(0, 2550)) +
  labs(x = NULL,
       y = "Costo entre personas beneficiarias",
       fill = NULL)  +
  theme_minimal() +
  theme(axis.text.y = element_markdown(),
        legend.position=c(.8,.4))
...
db %>% 
  ...
  geom_text(aes(x = fct_reorder(country, ord),
                y = ord,
                label = percent(ord, accuracy = .1)), hjust = -.2, size = 2.8) + 
  geom_hline(yintercept = 0.379, color = "grey40", size = .5,
             linetype = "dashed") +
  coord_flip() + 
  scale_y_continuous(label = percent_format(accuracy = 1),
                     breaks = c(seq(0,1.4,.2)), limits = c(0, 1.40)) +
